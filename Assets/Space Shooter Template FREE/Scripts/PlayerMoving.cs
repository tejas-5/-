...
public class PlayerMoving : MonoBehaviour {
    ...
    private void Update()
    {
        if (controlIsActive)
        {
            ...
            {
                Move(Vector2.up); // Move up
            }
            else if (Input.GetKeyDown(KeyCode.S))
            {
                Move(Vector2.down); // Move down
            }
            else if (Input.GetKeyDown(KeyCode.A))
            {
                Move(Vector2.left); // Move left
            }
            else if (Input.GetKeyDown(KeyCode.D))
            {
                Move(Vector2.right); // Move right
            }
    ...
    {
        // Move the player by adding force or changing position
